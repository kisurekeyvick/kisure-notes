 ...
 * (12) 使用webp图片
 * 
 * webp图片最初在2010年发布，目标是减少文件大小，但达到和JPEG格式相同的图片质量，
 * 希望能够减少图片档在网络上的发送时间。
 * webp图片无损比png图片无损的平均体积要小 20%~40%，并且图片质量用肉眼看几乎没什么差别
 * 
 * HTML提供了picture标签让我们可以在不同设备中使用不同的图片格式
 * 
 * HTML元素通过包含零或多个元素和一个  元素来为不同的显示/设备场景提供图像版本。浏览器会选择最匹配的子元素，
 * 如果没有匹配的，就选择  元素的 src 属性中的URL。然后，所选图像呈现在元素占据的空间中。
 * 
    <picture>
        <source srcset='....a.webp' type='image/webp'></source>
        <source srcset='....a.png' type='image/png'></source>
        <img src='....a.png'>
    </picture>

    picture标签包裹2个source标签,一个提供webp图片,通过srcset属性让浏览器从上到下选择可以支持的图片格式,
    如果浏览器不支持webp图片会只使用第二个source,会回退到png图片,如果浏览器不支持picture标签,
    会使用底部的img标签,同样也会生成一个png图片
