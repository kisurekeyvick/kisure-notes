 ...
 * 
 * (1) 对观察者模式的理解
 * 观察者模式(Observer)：一个对象（称为subject）维持一系列依赖于它的对象（称为observer），
 *              将有关状态的任何变更自动通知给它们（观察者）。
 * 
 * (2) 对发布/订阅模式的理解
 * 发布/订阅模式：基于一个主题/事件通道，希望接收通知的对象（称为subscriber）通过自定义事件订阅主题，
 *              被激活事件的对象（称为publisher）通过发布主题事件的方式被通知。
 * 
 * (3) 两种模式之间的差异
 *      - Observer模式要求观察者必须订阅内容改变的事件，定义了一个一对多的依赖关系
 *      - Publish/Subscribe模式使用了一个主题/事件通道，这个通道介于订阅着与发布者之间
 *      - 观察者模式里面观察者「被迫」执行内容改变事件（subject内容事件）；
 *          发布/订阅模式中，订阅着可以自定义事件处理程序；
...
// or
subscribe.notify('继续玩耍～');

/** 
 * 可以看出，每一个observer虽然也可以自定义自己的处理程序（update方法），
 * 但是观察者模式下，观察者们都是做同一类的事情的。
 */
